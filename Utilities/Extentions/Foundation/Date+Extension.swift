...
public extension Date {
    ...
    func getDateTimeToShow(withTime: Bool = false, relativeTime: Bool = false) -> String {
        if self.compare(.isToday) {
            if relativeTime {
                return self.toStringWithRelativeTime()
            }
            return self.getStringTime()
        } else if self.compare(.isYesterday) {
            if withTime {
                return "Yesterday, " + self.getStringTime()
            }
            return "Yesterday"
        } else {
            if withTime {
                return self.getStringDate(format: "dd-MM-yyyy hh:mm a")
            }
            return self.getStringDate(format: DateTimeFormat.MM_DD_YYYY.rawValue)
        }
    }

    func getStringDate(style: DateFormatter.Style = .medium, format: String? = nil) -> String {
        let dateFormatter = DateFormatter()
        if format != nil {
            dateFormatter.dateFormat = format! // "MMM dd"
        } else {
    ...
    func getStringTime(style: DateFormatter.Style = .short, relativeTime: Bool = false) -> String {

        if self.compare(.isToday) && relativeTime {
            return self.toStringWithRelativeTime()
        }
    ...
    func toStringWithRelativeTime(strings: [RelativeTimeStringType: String]? = nil) -> String {
        ...
        if sec < 60 {
            if sec < 10 {
                if isPast {
                    ...
                    return strings?[.nowFuture] ?? NSLocalizedString("in a few secs", comment: "Date format")
                }
            } else {
                let string: String
                if isPast {
                    string = strings?[.secondsPast] ?? NSLocalizedString("%.f secs ago", comment: "Date format")
                } else {
                    string = strings?[.secondsFuture] ?? NSLocalizedString("in %.f secs", comment: "Date format")
                }
        ...
        if min < 60 {
            if min == 1 {
                if isPast {
                    return strings?[.oneMinutePast] ?? NSLocalizedString("1 min ago", comment: "Date format")
                } else {
                    return strings?[.oneMinuteFuture] ?? NSLocalizedString("in 1 min", comment: "Date format")
                }
            } else {
                let string: String
                if isPast {
                    string = strings?[.minutesPast] ?? NSLocalizedString("%.f mins ago", comment: "Date format")
                } else {
                    string = strings?[.minutesFuture] ?? NSLocalizedString("in %.f mins", comment: "Date format")
                }
...
extension Date {
    ...
    func rounded(on amount: Int, _ component: Calendar.Component) -> Date {
        ...
        if let newDate = cal.date(byAdding: component, value: roundedValue - value, to: self) {
            return newDate.floorAllComponents(before: component)
        } else {
            return Date().floorAllComponents(before: component)
        }
    }
    ...
    func floorAllComponents(before component: Calendar.Component) -> Date {
        ...
        components.suffix(from: index + 1).forEach { roundComponent in
            ...
            if let dateGet = cal.date(byAdding: roundComponent, value: value, to: date) {
                date = dateGet
            }
